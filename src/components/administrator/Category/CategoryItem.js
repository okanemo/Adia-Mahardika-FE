...
const CategoryItem = ({ category, onSelectCategoryEdit, onSelectCategoryDelete }) => {
    ...
    return (
      ...
      <td className='manageCategory'>
        <button type="button" className="btn btn-sm btn-outline-delete" data-toggle='modal' data-target='#deleteModal' onClick={onClickDeleteCategory} data-dismiss='modal'>Delete</button>
        <button type="button" className="btn btn-sm btn-edit" data-toggle='modal' data-target='#editModal' onClick={onClickEditCategory}>Edit</button>
    </td>
